...
function renderCalendar() {
  ...
  switch (selectedView) {
    ...
    case "year":
      ...
      break;
    case "week":
      html = generateWeekView(currentWeekOffset);
      break;
    case "day":
...
function renderMonthView(year, month) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (hasEvents) {
      let innerHTML = `<div class="events-hover">${eventsForDay[0]}`;
...
function generateWeekView(weekOffset) {
  ...
  // Define hours dynamically based on 24-hour format
  var hours = [];
  for (var i = 0; i < 24; i++) {
      if (i === 0) {
          hours.push(""); // Inserting empty string for 24-hour format hover
      }
      hours.push((i < 10 ? "0" : "") + i + ":00"); // Inserting hours in the 24-hour format
  }
  ...
  hours.forEach(function(hour, index) {
      ...
      days.forEach(function(day, dayIndex) {
          ...
          if (index === 0) {
              ...
              // Fetch events for this date and include them in the cell
              const dateKey = `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date.getDate().toString().padStart(2, '0')}`;
              const storedNotes = JSON.parse(localStorage.getItem("weekevent1")) || {};
              const eventsForDay = storedNotes[dateKey] || [];
              eventsForDay.forEach(event => {
                  html += `<div style="background-color: #9f6d38; color:white;">${event.title}</div>`; // Display only the title in red color
              });
          }
          html += "</td>";
      });
...
function generateDayView(dayOffset) {
  // Define hours in 24-hour format
  var hours = [];
  for (var i = 0; i < 24; i++) {
      var hour = (i < 10 ? "0" : "") + i + ":00"; // Format hour in 24-hour format
      hours.push(hour);
  }

  // Get the current date
  var currentDate = new Date();
  currentDate.setDate(currentDate.getDate() + dayOffset); // Adjust date based on day offset
  ...
  // Set the formatted date and day in the header
  var headerHtml = '<th class="time-column" id="currentDate">' + formattedDate + " (" + dayOfWeek + ")" + "</th>";
  headerHtml += '<th colspan="4">Today</th>'; // Adjust the colspan according to the number of time slots
  ...
  hours.forEach(function(hour) {
      ...
      // Fetch events for this hour and include them in the cell
      const storedNotes = JSON.parse(localStorage.getItem("weekevent1")) || {};
      const eventsForHour = storedNotes[hour] || [];

      html += "<td colspan='4'>";
      eventsForHour.forEach(event => {
          html += "<div>" + event.title + "</div>"; // Display title
      });
      html += "</td>";
