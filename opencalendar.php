...
?>
<?php
// Retrieve the calendar ID from the URL
if(isset($_GET['id'])) {
    $id = $_GET['id'];
    // $name = $_SESSION['calendarName'];
    // var_dump($name);
    // Query to fetch calendar details
    $sql = "SELECT calendarName FROM created_calendar_details WHERE id = '$id'";
    $result = mysqli_query($conn, $sql);

    if ($result && mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $calendarName = $row['calendarName']; // Calendar name retrieved from the database
    } else {
        $calendarName = "Unknown"; // Default value if calendar name is not found
    }
} else {
    // If ID is not provided in the URL, set default values
...
<body>
  ...
  <i class='bx bx-user'><a class="navbar-brand"><?php echo $calendarName; ?></a></i>
...
<div id="update" class="modal">
  <div id="updateContent" class="modal-content">
    ...
    <div id="content">
        ...
        <form method="post" action="abc.php?id=<?php echo $id; ?>" onsubmit="addEvent('${dateKey}')">
...
<div id="settingModal" class="modal">
  ...
  <section class="home-section">
   ...
   <script src="calendarrendar1.js"></script>
   <script>
